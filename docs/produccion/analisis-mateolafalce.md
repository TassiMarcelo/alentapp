...
|---|----------|----------------|---------|--------------------|
| 1 | **Imagen base sin versión fija**. La etiqueta `node:20-alpine` es mutable: Docker Hub la reescribe con cada patch de Node.js o Alpine, por lo que dos builds del mismo Dockerfile hechos en momentos distintos pueden generar imágenes diferentes. Esto compromete la reproducibilidad y puede introducir cambios de comportamiento inesperados en producción. | `packages/api/Dockerfile:1` y `packages/web/Dockerfile:1` | Medio | [Pinear una versión exacta](https://docs.docker.com/build/building/best-practices/) del tag (p. ej. `node:20.14.0-alpine3.19`) o, mejor aún, fijar el digest `sha256` de la imagen. |
